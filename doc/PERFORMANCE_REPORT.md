...
**New file:** `backend/src/utils/cache.ts` — Simple TTL in-memory cache using `node-cache`

```typescript
import NodeCache from 'node-cache';
export const cache = new NodeCache({ stdTTL: 60, checkperiod: 120 });

export const cacheGet = <T>(key: string, fallback: () => Promise<T>): Promise<T> => {
  const cached = cache.get<T>(key);
  if (cached) return Promise.resolve(cached);
  return fallback().then(val => { cache.set(key, val); return val; });
};
```
...
2. **Database Migration** — Run `013_optimize_queries.sql` on production to add remaining indexes
