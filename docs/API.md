...
| Method | Path | Description |
|---|---|---|
| DELETE | `/auth/account` | Delete your account and all associated data |
| GET | `/auth/export-data` | Export all your data (GDPR/DPDP compliance) |
| POST | `/auth/push-token` | Register a push notification token |
...
| Method | Path | Description |
|---|---|---|
| GET | `/subscriptions/feature-flags` | Get public feature flags |
| GET | `/subscriptions/plans` | Get available subscription plans |
| POST | `/subscriptions/checkout/razorpay` | Create a Razorpay checkout order `{ period?: "monthly" \| "yearly" }` |
| POST | `/subscriptions/webhooks/razorpay` | Razorpay webhook receiver (no auth — called by Razorpay) |
...
| Method | Path | Description |
|---|---|---|
| GET | `/admin/stats` | System statistics (total users, children, devices, subscriptions) |
| GET | `/admin/users?page&limit` | List all users |
| GET | `/admin/users/:userId` | Get user details |
| PATCH | `/admin/users/:userId/subscription` | Update user subscription tier `{ tier: "FREE"\|"TRIAL"\|"PREMIUM" }` |
