# Kavach - Terms of Service

**Effective Date:** August 25, 2026  
**Last Updated:** August 25, 2026  
**App Name:** Kavach - Parental Control

---

## 1. Acceptance of Terms

By downloading, installing, or using Kavach ("the App"), you ("User", "you", "your") agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, do not use the App.

These Terms constitute a legally binding agreement between you and Kavach Technologies ("we", "us", "our").

We reserve the right to modify these Terms at any time. Material changes will be communicated via email and in-app notification. Your continued use of the App after such changes constitutes acceptance of the modified Terms.

---

## 2. Eligibility

### 2.1 Age Requirement
Kavach is available only to individuals who are:
- At least 18 years of age, OR
- The legal parent or guardian of a minor child

### 2.2 Parental/Guardian Use Only
- The App is designed exclusively for parents and legal guardians to monitor and manage their children's digital safety
- You must be the legal parent or guardian of any child profile you create
- You must have legal authority to consent to data collection on behalf of the child

### 2.3 Account Registration
- You must provide accurate and complete information during registration
- You are responsible for maintaining the confidentiality of your account credentials
- You must notify us immediately of any unauthorized use of your account
- One account per parent; sharing accounts is prohibited

---

## 3. Account Responsibilities

### 3.1 Credential Security
- You are responsible for safeguarding your password and PIN
- Use a strong, unique password
- Enable biometric authentication where available
- Never share your credentials with others

### 3.2 Session Management
- Sessions are managed via secure refresh tokens
- You may sign out of all devices via the "Sign Out All Devices" feature
- Suspicious activity will result in automatic session revocation

### 3.3 Account Changes
- You may update your profile information at any time via `PUT /auth/profile`
- You may change your password via `PUT /auth/password`
- You may set or rotate your parental PIN via `PUT /auth/pin`

### 3.4 Account Deletion
- You may delete your account and all associated data via `DELETE /auth/account`
- Account deletion is permanent and irreversible
- All child profiles, devices, and associated data will be permanently removed
- You may also export your data before deletion via `GET /auth/export-data`

---

## 4. Acceptable Use

### 4.1 Permitted Uses
You may use Kavach to:
- Create and manage child profiles for your own children
- Set and enforce screen-time limits
- Block applications on your child's device
- Monitor your child's location (with appropriate consent)
- Receive alerts for safety concerns
- Manage geofences for location-based alerts
- Track your child's mood and wellness

### 4.2 Prohibited Uses
You must NOT:
- Use the App to monitor anyone other than your own minor children
- Use the App for any illegal purpose
- Attempt to bypass or circumvent security measures
- Attempt to access another user's account or data
- Use the App to stalk, harass, or harm any individual
- Reverse engineer, decompile, or disassemble the App
- Remove, alter, or obscure any proprietary notices
- Use the App in violation of any applicable law
- Share your account credentials with unauthorized parties
- Use automated tools to access the App without written permission

### 4.3 Child Data Ethics
- Collect only data necessary for child safety purposes
- Do not use collected data to punish or emotionally harm your child
- Use the App as a tool for safety, not surveillance
- Maintain open communication with your child about monitoring
- Respect your child's developing need for privacy as they mature

---

## 5. Service Description

### 5.1 Core Features
Kavach provides the following parental control features:
- **App Blocking:** Block specific applications on your child's device
- **Screen Time Management:** Set daily screen-time limits and per-app usage caps
- **Location Tracking:** Real-time GPS tracking with location history
- **Geofencing:** Create safe zones with entry/exit alerts
- **Communication Monitoring:** SMS and call log monitoring with keyword-based cyberbullying detection
- **Emergency SOS:** One-touch emergency alert system
- **Device Health Monitoring:** Battery, storage, and security status
- **Mood Tracking:** Self-reported mood and wellness tracking
- **Multi-Device Support:** Manage multiple child devices from one dashboard
- **Co-Guardian Sharing:** Share child monitoring with another trusted adult

### 5.2 Service Limitations
- The App requires an active internet connection on the child's device
- Location accuracy depends on GPS signal and device capabilities
- App blocking requires Device Administrator privileges on Android
- The App does not guarantee 100% bypass prevention
- Real-time monitoring depends on device connectivity
- Some features may not be available on all Android versions

---

## 6. Intellectual Property

### 6.1 Ownership
- Kavach and all related technologies, including the App, backend services, APIs, and documentation, are owned by Kavach Technologies
- All trademarks, logos, and brand names are our exclusive property

### 6.2 License
- We grant you a limited, non-exclusive, non-transferable, revocable license to use the App
- This license is for personal, non-commercial use only
- You may not sublicense, sell, or distribute the App

### 6.3 User Content
- You retain ownership of data you input into the App
- By using the App, you grant us a limited license to process your data as described in our Privacy Policy
- You are responsible for the accuracy of data you provide

---

## 7. Privacy and Data Protection

### 7.1 Privacy Policy
Your use of the App is also governed by our Privacy Policy, which is incorporated into these Terms by reference.

### 7.2 DPDP Act Compliance
- We comply with the Digital Personal Data Protection Act, 2023
- We obtain verifiable parental consent before collecting child data
- You may access, correct, or delete your child's data at any time
- Data is retained only as long as necessary (see Privacy Policy for retention periods)

### 7.3 Data Security
- All data is encrypted in transit (TLS 1.2+) and at rest
- Access to child data is restricted to verified parents only
- All data access is logged in an immutable audit trail
- We implement industry-standard security measures

---

## 8. Payment and Subscriptions

### 8.1 Pricing
- Kavach may offer free and premium subscription tiers
- Pricing is displayed in the App and on our website
- All prices are in Indian Rupees (INR) unless otherwise stated

### 8.2 Billing
- Subscriptions are billed through Razorpay (web-based payment processing)
- Payment is processed by Razorpay; we do not store payment card information
- Subscriptions auto-renew unless cancelled at least 24 hours before the renewal date

### 8.3 Refunds
- Refund requests are handled through Razorpay's refund policy
- Contact us at support@kavach.app for billing inquiries

### 8.4 Price Changes
- We may change subscription prices with 30 days' notice
- Price changes take effect at the next billing cycle

---

## 9. Limitation of Liability

### 9.1 Disclaimer of Warranties
- THE APP IS PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT WARRANTIES OF ANY KIND
- WE DO NOT WARRANT THAT THE APP WILL BE UNINTERRUPTED, ERROR-FREE, OR SECURE
- WE DO NOT WARRANT THAT THE APP WILL PREVENT ALL BYPASSES OR SECURITY BREACHES
- WE DO NOT WARRANT THE ACCURACY OF LOCATION DATA OR MONITORING INFORMATION

### 9.2 Limitation of Damages
TO THE MAXIMUM EXTENT PERMITTED BY LAW:
- WE SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES
- OUR TOTAL LIABILITY SHALL NOT EXCEED THE AMOUNT PAID BY YOU IN THE 12 MONTHS PRECEDING THE CLAIM
- WE SHALL NOT BE LIABLE FOR ANY LOSS OF DATA, BUSINESS INTERRUPTION, OR COMPUTER DAMAGE

### 9.3 Specific Exclusions
We are not liable for:
- Damages resulting from unauthorized access to your account due to your negligence
- Damages resulting from device malfunctions or operating system issues
- Damages resulting from the child's ability to bypass controls
- Damages resulting from third-party service disruptions
- Loss of data due to force majeure events

---

## 10. Indemnification

You agree to indemnify, defend, and hold harmless Kavach Technologies, its officers, directors, employees, and agents from any claims, liabilities, damages, or expenses arising from:
- Your use of the App
- Your violation of these Terms
- Your violation of any applicable law
- Your violation of any third-party rights
- Any data you input into the App

---

## 11. Termination

### 11.1 Termination by You
- You may stop using the App at any time
- You may delete your account by contacting support
- Uninstalling the App does not delete your account or data

### 11.2 Termination by Us
We may suspend or terminate your account if:
- You violate these Terms
- You engage in prohibited use
- We are required to do so by law
- We discontinue the App (with 90 days' notice)

### 11.3 Effect of Termination
- Upon termination, your right to use the App ceases immediately
- We may retain data as required by law or for legitimate business purposes
- Sections 6, 7, 9, 10, 12, and 13 survive termination

---

## 12. Dispute Resolution

### 12.1 Governing Law
These Terms are governed by the laws of India, including the Information Technology Act, 2000 and the Digital Personal Data Protection Act, 2023.

### 12.2 Jurisdiction
Any disputes arising from these Terms shall be subject to the exclusive jurisdiction of the courts in [City], India.

### 12.3 Arbitration
Before initiating litigation, the parties agree to attempt to resolve disputes through:
1. **Informal Negotiation:** Contact support@kavach.app
2. **Mediation:** Through a mutually agreed mediator in [City], India
3. **Arbitration:** If unresolved within 60 days, through arbitration under the Arbitration and Conciliation Act, 1996

### 12.4 Class Action Waiver
You agree to resolve disputes on an individual basis and waive any right to participate in class actions or class-wide arbitration.

---

## 13. General Provisions

### 13.1 Entire Agreement
These Terms, together with the Privacy Policy, constitute the entire agreement between you and Kavach Technologies.

### 13.2 Severability
If any provision of these Terms is found to be unenforceable, the remaining provisions shall remain in full force and effect.

### 13.3 Waiver
Our failure to enforce any provision of these Terms shall not constitute a waiver of that provision.

### 13.4 Assignment
You may not assign these Terms without our written consent. We may assign these Terms without restriction.

### 13.5 Force Majeure
We are not liable for delays or failures caused by events beyond our reasonable control, including natural disasters, government actions, or infrastructure failures.

### 13.6 Notices
Notices to you may be sent via email, in-app notification, or posted on our website. Notices to us should be sent to legal@kavach.app.

---

## 14. Contact Us

For questions about these Terms:

**Email:** legal@kavach.app  
**Support:** support@kavach.app  
**Address:** Kavach Technologies, [Office Address, India]

---

## 15. Grievance Redressal

In accordance with the Information Technology Act, 2000 and the DPDP Act, 2023:

**Grievance Officer:**  
Name: [Designated Grievance Officer Name]  
Email: grievance@kavach.app  
Phone: +91-XXXXXXXXXX  
Address: [Office Address, India]

**Response Time:** We will acknowledge your grievance within 24 hours and aim to resolve it within 30 days.

If you are not satisfied with our response, you may escalate your grievance to the Data Protection Board of India (DPBI).
