...
Choose your deployment mode: Development, Production, or Local.

#### **Step 2: Backend Configuration**
Generates JWT secrets, database config, rate limiting, and CORS settings.

#### **Step 3: Frontend Configuration**
Configures API URL, Socket.io URL, and Mapbox token.

#### **Step 4: Android Configuration**
Sets API base URL and certificate pinning for the Android app.

#### **Step 5: Deploy**
Generates a complete deployment script that handles everything:
- Checks prerequisites (Node.js, Docker)
- Writes .env files
- Starts database via Docker
...
If you prefer not to use the wizard, follow these manual steps. Otherwise, skip to the next section.
...
### **Android BuildConfig**
Configure via `android/local.properties` or Gradle build arguments:
...
2. Run: `docker compose -f deploy/docker-compose.yml up` from the repo root
...
4. Place it in the `kavach/android/app/` directory
...
#### **Android ✅**
- [ ] API_BASE_URL set correctly in Gradle config
- [ ] FCM_ENABLED matches your choice
- [ ] Certificate pins set (release only)
- [ ] `google-services.json` in `android/app/` (if Firebase enabled)
...
- If using Docker: `docker compose -f deploy/docker-compose.yml up` from repo root
...
- Ensure `google-services.json` is in `kavach/android/app/` if Firebase enabled
