...
- `DELETE /auth/account` - Self-service account deletion (available to all users)

### Recommendations
1. ~~Implement self-service account deletion endpoint~~ — **DONE** (`DELETE /auth/account`)
2. Add data export before deletion (right to data portability) — `GET /auth/export-data` available
