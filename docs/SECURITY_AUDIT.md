...
**Severity:** MEDIUM  
**Status:** RESOLVED — Token family rotation is now implemented in the auth middleware. Refresh tokens are rotated on each use, and token families are tracked to detect reuse.
...
**Severity:** MEDIUM  
**Status:** IMPROVED — OpenAPI spec has been updated with security definitions for all authenticated endpoints. Subscription and admin endpoints are pending addition.
