...
function createWrapper(initialPath = '/children/child-1/sos') {
  ...
  return ({ children }: { children: React.ReactNode }) => (
    <QueryClientProvider client={queryClient}>
      <MemoryRouter initialEntries={[initialPath]}>
        <Routes>
          <Route path="/children/:childId/sos" element={children} />
        </Routes>
      </MemoryRouter>
    </QueryClientProvider>
  );
...
describe('SOSPage', () => {
  ...
  it('renders SOS page with loading state', () => {
    const { container, unmount } = render(<SOSPage />, { wrapper: createWrapper() });
